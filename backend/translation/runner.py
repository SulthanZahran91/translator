...
from backend.translation.orchestrator import TranslationOrchestrator, TranslationProgress, TranslationPhase
...
class JobRunner:
    ...
    async def run(self):
        ...
        with open("/tmp/debug_logs.txt", "a") as f:
            f.write(f"[{datetime.utcnow()}] JobRunner.run started for job {self.job_id}\n")
            
        async with async_session_factory() as db:
            with open("/tmp/debug_logs.txt", "a") as f:
                f.write(f"[{datetime.utcnow()}] DB session created\n")
            try:
                # Load job
                job = await self._get_job(db)
                with open("/tmp/debug_logs.txt", "a") as f:
                    f.write(f"[{datetime.utcnow()}] Job loaded: {job}\n")
                ...
                # Start processing
                with open("/tmp/debug_logs.txt", "a") as f:
                    f.write(f"[{datetime.utcnow()}] Updating status to PROCESSING\n")
                await self._update_status(db, job, JobStatus.PROCESSING, "Starting translation job")
                with open("/tmp/debug_logs.txt", "a") as f:
                    f.write(f"[{datetime.utcnow()}] Status updated\n")
                ...
                # Create a wrapper for the sync callback to run async db updates
                # Note: This is a bit tricky since the callback is sync.
                # We might need to just update local state and flush periodically, 
                # or use a separate task for updates.
                # For now, we'll rely on the orchestrator's internal progress tracking 
                # or maybe poll or just update at the end of each unit if possible.
                # Actually, TranslationOrchestrator.translate_units is async, so we can pass a callback
                # but the callback definition in orchestrator is sync: Callable[[TranslationProgress], None]
                # We will just log major events here.
                ...
                with open("/tmp/debug_logs.txt", "a") as f:
                    f.write(f"[{datetime.utcnow()}] JobRunner failed for job {self.job_id}: {e}\n")
