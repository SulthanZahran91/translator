...
def _pt_to_float(pt_value: Any) -> float | None:
    """Convert Pt/EMU value to float points.
    
    python-docx stores many values in EMUs (English Metric Units).
    1 point = 12700 EMUs.
    """
    if pt_value is None:
        return None
...
def _inches_to_float(inches_value: Any) -> float | None:
    ...
    def _parse_cell(self, docx_cell: _Cell) -> TableCell:
        ...
        if tcPr is not None:
            ...
            if vAlign is not None:
                v_alignment = vAlign.get(qn("w:val"))
        else:
            v_alignment = None
